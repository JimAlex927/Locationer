...
<div align="center">
Locationer - 用于 Android 8.0+ 的无需 ROOT 权限的虚拟定位 APP
</div>

## 简介
&emsp;&emsp;Locationer 是一个基于 Android 调试 API、MapLibre 和 OpenFreeMap 实现的安卓定位修改工具，并且同时实现了一个可以自由控制移动的摇杆。使用 Locationer，不需要 ROOT 权限就可以修改自己的当前位置以及模拟移动，也不需要申请商业地图密钥。
...
## 警告一
&emsp;&emsp;**最近，有网友直接使用 Locationer 源码后改名并添加广告（除了加广告，功能没有任何改变），但是，没有按照 GPLv3 协议的要求进行开源（我已经联系过该网友进行了提醒，但并没有收到回复），在此提醒：**
1. **开源 ≠ 白嫖，请遵循开源协议**
2. **GPL 的法律效力在国内相关诉讼案例很多，请自行搜索，权衡利弊。Locationer 保留追究相关侵权人员法律责任的所有权利！**
3. **开源不易，且行且珍惜**

## 警告二
&emsp;&emsp;**最近，有很多人将 Locationer 用在校园运动类 APP（包括但不限于闪动校园、TakeTwo、运动世界校园等）中作弊，开发者也收到了很多人提问为何 Locationer 定位并不起作用或者寻求对 Locationer 的改进，在此提醒：**
1. **Locationer 不支持任何校园运动类 APP 的作弊行为**
2. **Locationer 开发者也不赞同采用任何形式在校园运动中作弊**
...
1. 该 APP 仅仅是为了学习 Android 与地图开发的实现方法，请勿用于游戏作弊！
2. 该 APP 仅仅是为了学习 Android 与地图开发的实现方法，请勿用于游戏作弊！
3. 该 APP 仅仅是为了学习 Android 与地图开发的实现方法，请勿用于游戏作弊！
...
## 参考
&emsp;&emsp;由于本人也是个新手，纯属业余瞎搞，因此，在写 Locationer 的过程中，参考了很多网友分享的技术文章、示例代码等。包括但不限于以下列出的几个：
...
A：经过测试，Locationer 可以在鸿蒙系统上正常运行。
